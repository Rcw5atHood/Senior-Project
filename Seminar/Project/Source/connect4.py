...
import random
import sys
from constants import BLACK, RED, NAVY, YELLOW, LIGHT_GRAY, screen_width, screen_height,\
    CELL_SIZE, GRID_WIDTH, GRID_HEIGHT, GRID_MARGIN_X,GRID_MARGIN_Y, FONT, FPS
pygame.init()
# Set up the clock
...
def draw_winning_tiles(winning_tiles, color):
...
def flash_winning_tiles(winning_tiles):
    for _ in range(5):
        draw_winning_tiles(winning_tiles, LIGHT_GRAY)
        pygame.display.flip()
        pygame.time.delay(200)
        draw_winning_tiles(winning_tiles, get_color(winner))
        pygame.display.flip()
        pygame.time.delay(200)
