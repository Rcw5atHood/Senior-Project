...
def draw_frames():
    ...
    side_text_frame = pygame.Rect(side_text_x - 10, side_text_y - 10, side_text_width + 20, side_text_height + 20)
...
def check_winner(player):
    ...
    for col in range(len(game_state[0])):
        for row in range(len(game_state) - 3):
            if game_state[row][col] == player and game_state[row + 1][col] == player and game_state[row + 2][
                col] == player and game_state[row + 3][col] == player:
                return True

    # Check diagonal (bottom-left to top-right)
    for row in range(len(game_state) - 3):
        for col in range(len(game_state[0]) - 3):
            if game_state[row][col] == player and game_state[row + 1][col + 1] == player and game_state[row + 2][
                col + 2] == player and game_state[row + 3][col + 3] == player:
                return True
    # Check diagonal (top-left to bottom-right)
    for row in range(3, len(game_state)):
        for col in range(len(game_state[0]) - 3):
            if game_state[row][col] == player and game_state[row - 1][col + 1] == player and game_state[row - 2][
                col + 2] == player and game_state[row - 3][col + 3] == player:
                return True
...
def draw_side_text(current_player, winner):
    if winner:
        message = "Player 1 wins!" if winner == 1 else "CPU wins!"
    elif current_player == 1:
        message = "Player one, select your placement"
    else:
...
# Game loop
def pygconnect4_main(window_caption="Connect Four"):
